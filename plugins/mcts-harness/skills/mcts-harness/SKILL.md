...
B. 壁打ちしてからプランを固めたい（修正・改善・アイデア整理）
C. 既存の実装プランがある（フェーズ2から開始）
```
...
## フェーズ 1.5: 壁打ち（ブレインストーミング）

**フェーズ0でB（壁打ちから始める）を選択した場合のみ実行。**
...
```
何について壁打ちしたいですか？
現状・やりたいこと・モヤっていることなど、思いつくままに話してください。

（「プランに進んで」「固めよう」と言えばプランモードに移行します）
```
...
1. **Claudeの役割**: 聞き役・整理役に徹する
   - アイデアや論点を整理して返す
   - 選択肢・トレードオフがあれば提示する
   - 結論を急がず、ユーザーの思考を引き出す

2. **終了判定**: ユーザーが以下のいずれかを言ったら壁打ち終了
   - 「プランに進んで」「固めよう」「プランモードへ」
   - その他、プランニングへの移行を示す発言

### 1.5c: 壁打ちサマリーの生成

壁打ち終了後、対話内容を以下の形式でまとめる（内部メモとしてフェーズ2に引き渡す）:
...
このサマリーをコンテキストとしてフェーズ2へ進む（wantree.ymlは生成しない）。
...
- 壁打ちサマリー（フェーズ1.5を経た場合）
- 既存のコード・ファイル構成（フェーズ0でCを選択した場合）
